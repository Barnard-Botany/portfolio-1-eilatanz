...
# (change "Child" to "Height") not best way to do this - need parents' midparent value to not group with offspring - it's paired, etc. T Test
...
# Is this statistically significant? pvalue = .3735, no, not stat sig
#if you hit x the output can be erased, can be got back by selecting run again 
...
```{r}
# Let's plot the histograms can load library info and go to packages and search for lattice, which you can install if not there so it will appear. Can also install with the cmd install(thingy)
...
# Plot the 95% confidence interval (2.5% - 97.5%) confidence interval (2x stand dev around mean) 
